...
export default class Form extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({book:b.book, fieldIdBook2:id, books:bk.books, fieldPay:b.book.price})
  ...
  render() {
    return (
      ...
      backgroundSize: "cover" , padding: "50px", borderRadius: '5px', fontWeight: "bold", marginLeft: "-100"}}>
        <h3 style={{color: "#007bff"}}>Realizar petición a {this.state.book.title}</h3>
        ...
        <div class="form-group row">
          ...
          {this.state.errorField.indexOf("action") != -1 ? 
                <p class='text-danger'>{this.state.errorMessages[this.state.errorField.indexOf("action")]}</p>
              :
                <p></p>
              }
          </div>
        ...
        {this.state.sell != null ?
          this.state.sell == false ?
              <div class="form-group row">
                  ...
                  <div class="col-sm-9">
                      ...
                      {this.state.errorField.indexOf("idBook1") != -1 ? 
                        <p class='text-danger'>{this.state.errorMessages[this.state.errorField.indexOf("idBook1")]}</p>
                      :
                        <p></p>
                      }
                      {this.state.noBooks ?
                        <div><br></br><p class='text-danger'>*No tiene libros para intercambiar. <a href="/books/new" class="btn btn-primary">Sube uno ahora!</a></p></div>
                      :
                        <p></p>
                      }
                  </div>
            ...
            <div>
              <div class="form-group row">
                <label for="firstName" class="col-sm-3 col-form-label">Precio original:</label>
                <div class="col-sm-9">
                ...
                <div class="col-sm-9">
                  <input type="number" class="form-control"
                    ...
                    {this.state.errorField.indexOf("pay") != -1 ? 
                      <p class='text-danger'>{this.state.errorMessages[this.state.errorField.indexOf("pay")]}</p>
                    :
                      <p></p>
                    }
                </div>
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-9">
            <input type="text" class="form-control"
              ...
              onChange={(event)=>this.setState({fieldComment:event.target.value})} disabled={this.state.noBooks && this.state.sell == false}/>
              {this.state.errorField.indexOf("comment") != -1 ? 
                <p class='text-danger'>{this.state.errorMessages[this.state.errorField.indexOf("comment")]}</p>
              :
                <p></p>
              }
          </div>
        ...
        <div class="form-group row">
            <div class="col-sm-6" >
              <br></br>
        <button onClick={()=>this.onClickSave()} class="btn btn-primary" style={{float:"left"}} type="submit"  
        disabled={this.state.noBooks && (this.state.sell == false || this.state.sell == null)}>Enviar</button>
            </div>
        </div>
      </div>
